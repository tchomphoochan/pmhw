...
Proof.
  ...
  - unfold R_pm in *.
    ...
    simpl in *; intuition.
    apply Permutation_sym in H0, H1.
    constructor; simpl in *; eapply perm_trans; eassumption.
  - unfold R_pm in *.
  ...
  - unfold R_pm in *; unfold ValidPmState in *.
    ...
    eapply SpecStart with (s' := {| SpecQueued := Queued1 ++ Renamed1 ++ Scheduled1;
                                    SpecRunning := Running1 |});
      simpl; subst.
    + apply IHpm_trace; intuition; inversion H9; try assumption.
  ...
  - unfold R_pm in *; unfold ValidPmState in *.
    ...
    eapply SpecFinish with (s' := {| SpecQueued := Queued1 ++ Renamed1 ++ Scheduled1;
                                      SpecRunning := Running1 |});
      simpl; subst.
    + apply IHpm_trace; try rewrite Forall_app in *; intuition.
      inversion H1.
      assumption.
    + apply Permutation_sym.
      assumption.
